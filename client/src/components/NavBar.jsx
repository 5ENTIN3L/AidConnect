...
function NavBar({ activePage }) {
    ...
    const navLinks = allLinks.filter(link => canAccess(link.id));
    ...
    return (
        <>
            ...
            <nav
                ...
                onClick={() => { if (scrolled && !isMenuOpen) setIsMenuOpen(true); }}
            >
                {(!scrolled || isMenuOpen) ? (
                    <div className={`flex ${scrolled ? 'flex-col gap-6' : 'items-center justify-between'}`}>
                        <div className="flex items-center gap-3 justify-between">
                            ...
                            {scrolled && isMenuOpen && (
                                <button
                                    ...
                                    className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-red-100 hover:text-red-500 transition-colors"
                                >
                        ...
                        <div className={`flex ${scrolled ? 'flex-col items-start gap-4' : 'items-center gap-6'}`}>
                            {navLinks.map((link) => (
                                <button
                                    key={link.id}
                                    onClick={() => { window.navigateTo(link.id); setIsMenuOpen(false); }}
                                    className={`relative font-medium transition-all group overflow-hidden ${scrolled ? 'w-full text-left flex px-2 py-1' : ''} ${
                                        activePage === link.id
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                        <svg className="w-6 h-6 text-gray-700 dark:text-gray-300 hover:text-blue-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
